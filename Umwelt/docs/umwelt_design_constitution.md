...
- Merging happens only **inside** a neuron — faithfully, in its dendrite (an input-collecting tree). **Provisional simplification (MVP) — re-labeled 2026-06-02, NOT a settled call:** collapse the dendrite to the soma; inputs land directly on the neuron cell. Consequence: fan-in (inputs to one neuron) is capped by free neighbor cells (~6); for more, the player builds a funnel of relay neurons — reconstructing a dendrite *out of neurons* (same spirit as building OFF-cells). This *omits* a real structure (allowed in principle); it never *fabricates* — the hard line is that different neurons' signals never combine in a wire (no shared-conductor via). **Nuance (why provisional):** the natural form — an inward-collecting dendritic tree — may itself be a *good puzzle* (spatial aggregation of inputs), and we collapsed it **before characterizing that fun/cost** — i.e. a 删繁 (prune) done without earning its place (see the Decision heuristic's three-operations rule). It is not pure loss: the collapse *forces* the emergent "relay funnel" interest. Conclusion: **revisit once the dendrite-tree's puzzle value is characterized — an IOU, not a redo now.** Upgrade path if the in/out asymmetry bites: give neurons an inward dendrite tree to symmetrize.
...
- Plasticity is a per-edge property (`plastic`, `mod_source`). MVP allows it anywhere (a simplification); leave room to later gate *which* edges may be plastic by node type (mushroom-body locality). **Rule (F-LEARN, 2026-06-02): `Δw = −η·pre·mod` — two-factor, presynaptic, depression-dominant** (de-fabricated from the old `pre·post·mod`; matches dopamine-gated KC→MBON, Hige et al. 2015, *Neuron* 88:985). Behavioral direction comes from the player's MBON balance structure, not potentiation. Spec: `docs/superpowers/specs/2026-06-02-ant-circuit-roadmap-design.md` §5 F-LEARN; rationale in CLAUDE.md「可塑突触设计」. **Code unchanged (TODO: dual-track JS sim + Bevy eval, then rerun golden oracle ≤1e-5).**
...
**Simplification is three different operations — keep them distinct (added 2026-06-02):**
- **Fabricate (虚构)** — add what biology doesn't have (a function/structure invented from nothing). **Never.** This is the costume the Decision Protocol hunts.
- **De-fabricate (去虚构)** — strip a costume that doesn't belong to the structure (e.g. the `post` factor on mushroom-body plasticity — real KC→MBON learning is presynaptic, dopamine-gated, post-independent; Hige et al. 2015). **Anytime, immediately** — this is correction, not simplification.
- **Prune (删繁)** — omit a *real* natural structure (e.g. collapsing the dendrite tree to the soma, §2). **Only after** you can say what fun that structure holds and what it costs, and judge the cost not worth it.

**Principle — natural solution first; pruning must earn its place.** Until you can state *what fun is removed and what cost is saved*, the natural complete form is the default, and any omission carries an **IOU** to revisit. So the heuristic gains a first question: **before asking whether to simplify, ask what the most natural complete solution is, and what its fun and cost are.** This is the meta-law behind several standing decisions — *build the whole ant as an integration test* (not a clean isolated puzzle), *design the richest mid-game puzzle before backfilling a tutorial*, *don't build the tutorial first* — and it is why the dendrite collapse (§2) is now marked **provisional**, not settled. (Costs are paid for honesty without fear — that is who we are.)

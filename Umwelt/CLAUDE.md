...
格子的体积将来用于**体块组织**（肌肉、血管等）；神经轴突是沿格点阵的**棱**走线（编辑器 v2 已按此渲染：胞体落格、连线走棱、不填格）。已知的将来对齐点：一旦体块占了格，no-overlap 的单位会从"格"（现在宪法 §2 / memory `umwelt-grid-atomicity`）变为"棱"。**现在不做体块、后端照跑，不要为此重构路由** —— 留到将来跟体块一起对齐。
